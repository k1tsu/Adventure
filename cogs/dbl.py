    ...
    while True:
        ...
        if player:
            ...
            if not payload['isWeekend']:
                exp = random.randint(player.exp_to_next_level() // 16, player.exp_to_next_level() // 4)
                gold = random.randint(1000, 2000)
            else:
                exp = random.randint(player.exp_to_next_level() // 8, player.exp_to_next_level() // 2)
